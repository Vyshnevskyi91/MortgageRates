...
def schedule_scraping():
    ...
    schedule_time = f"10:{random_minute:02d}"
...
# Keep the script running and reschedule daily
while True:
    schedule.run_pending()
    time.sleep(10)
    
    # Reschedule at midnight with a delay to avoid multiple triggers
    current_time = time.localtime()
    if current_time.tm_hour == 0 and current_time.tm_min == 0:
        schedule.clear()  # Clear previous schedules
        schedule_scraping()  # Reschedule with a new random minute
        time.sleep(60)  # Pause for 1 minute to avoid repeated rescheduling